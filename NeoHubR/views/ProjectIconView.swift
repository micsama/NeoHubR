...
struct ProjectIconView: View {
    ...
    var body: some View {
        ZStack {
            if let descriptor = entry.iconDescriptor {
                ...
                Image(systemName: entry.isSession ? "doc.text.fill" : fallbackSystemName)
            }
        }
    ...
    private var iconStyle: AnyShapeStyle {
        if isInvalid { return AnyShapeStyle(.tertiary) }
        return AnyShapeStyle(entry.customColor ?? fallbackColor)
    }
}
